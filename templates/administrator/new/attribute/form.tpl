...
{block name=content}
        ...
        <div class="tab-content">	
            ...
            <div class="tab-pane active" id="general">
                <div class="panel-body">
                    ...
                    <fieldset>
                        <legend>{translate('form_label_attribute_values')}</legend>
                        <table id="attribute-value" class="table table-striped table-bordered table-hover">
                            <thead>
                                <tr>
                                    <td class="text-left required">{translate('form_label_attribute_value_name')}</td>
                                    <td class="text-right">{translate('form_label_attribute_value_filter')}</td>
                                    <td class="text-right">{translate('form_label_attribute_value_custom')}</td>
                                    <td class="text-right">{translate('form_label_attribute_value_sort_order')}</td>
                                    <td></td>
                                </tr>
                            </thead>
                            <tbody>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="4"></td>
                                    <td class="text-right"><button type="button" onclick="addAttributeValue();" data-toggle="tooltip" title="Add Attribute Value" class="btn btn-primary"><i class="icon-plus2"></i></button></td>
                                </tr>
                            </tfoot>
                        </table>
                    </fieldset>
    ...
    <script>
        var form_labels = {
            attribute_value_name: "{translate('form_label_attribute_value_name')}",
            value: "{translate('form_label_attribute_value_value')}",
            sort_order: "{translate('form_label_attribute_value_sort_order')}",
            attribute_value_add: "{translate('form_label_attribute_value_add')}",
        };
        var admin_theme = "{$admin_theme}";
        var slugGeneratorUrl = "{site_url_multi($admin_url)}/attribute/slugGenerator";
        var moduleUrl = "{site_url_multi($admin_url)}/attribute";
        var languages = $.parseJSON('{$languages|json_encode}');
        var item_id = "item_id";
        // Attribute data
        var attribute_value = $.parseJSON('{$attribute_value|json_encode}');
        // Attribute data end 
        {literal}
        // Slug generator
        ...
        function addAttributeValue() {
            ...
            Object.keys(languages).forEach(function(key) {
                ...
                html += '    <span class="input-group-addon"><img src="'+admin_theme+'/global_assets/images/flags/'+language.code+'.png" title="'+language.name+'" /></span><input type="text" name="attribute_value[' + attribute_value_row + '][attribute_value_description]['+language.id+'][name]" value="" placeholder="'+form_labels.attribute_value_name+'" class="form-control" />';
            });
            html += '    </div>';
            html += '  </td>';
            html += '  <td class="text-right"><input type="checkbox" name="attribute_value[' + attribute_value_row + '][filter]" value="1" class="styled" /></td>';
            html += '  <td class="text-right"><input type="checkbox" name="attribute_value[' + attribute_value_row + '][custom]" value="1" class="styled" /></td>';
           html += '  <td class="text-right"><input type="text" name="attribute_value[' + attribute_value_row + '][sort_order]" value="" placeholder="'+form_labels.sort_order+'" class="form-control" /></td>';
            html += '  <td class="text-right"><button type="button" onclick="$(\'#attribute-value-row' + attribute_value_row + '\').remove();" data-toggle="tooltip" title="Remove" class="btn btn-danger"><i class="icon-minus2"></i></button></td>';
            html += '</tr>';

            $('#attribute-value tbody').append(html);

            attribute_value_row++;
            $('.styled').uniform('update');
        }

        if(attribute_value) {
            Object.keys(attribute_value).forEach(function(value_key) {
                let element = attribute_value[value_key];
                console.log(element);
                html  = '<tr id="attribute-value-row' + attribute_value_row + '">';
                html += '  <td class="text-left"><input type="hidden" name="attribute_value[' + attribute_value_row + '][attribute_value_id]" value="'+element.id+'" />';
                html += '    <div class="input-group">';
                Object.keys(languages).forEach(function(key) {
                    let language = languages[key];
                    let language_index  = language.id;
                    html += '    <span class="input-group-addon"><img src="'+admin_theme+'/global_assets/images/flags/'+language.code+'.png" title="English" /></span><input type="text" name="attribute_value[' + attribute_value_row + '][attribute_value_description]['+language_index+'][name]" value="'+element.attribute_value_description[language_index].name+'" placeholder="Attribute Value Name" class="form-control" />';
                });
                html += '    </div>';
                html += '  </td>';
                 html += '  <td class="text-right"><input type="checkbox" name="attribute_value[' + attribute_value_row + '][filter]" value="1" class="styled"';
                 if(element.filter == 1)
                {
                     html += 'checked="checked"';
                }
                html += ' /></td>';
                html += '  <td class="text-right"><input type="checkbox" name="attribute_value[' + attribute_value_row + '][custom]" value="1" class="styled custom_row"';
                if(element.custom == 1)
                {
                     html += 'checked="checked"';
                }
                html += ' /></td>';
                html += '  <td class="text-right"><input type="text" name="attribute_value[' + attribute_value_row + '][sort_order]" value="'+element.sort_order+'"  placeholder="'+form_labels.sort_order+'" class="form-control custom_row" /></td>';
                html += '  <td class="text-right"><button type="button" onclick="$(\'#attribute-value-row' + attribute_value_row + '\').remove();" data-toggle="tooltip" title="Remove" class="btn btn-danger"><i class="icon-minus2"></i></button></td>';
                html += '</tr>';
        ...
        $('.custom_row').change(function() {
           
            if(this.checked) {
                $(this).parent().parent().parent().next().next().find('button').hide();
            }
            else
            {
                 $(this).parent().parent().parent().next().next().find('button').show();
            }
        });

        $('.custom_row').trigger('change');
