...
class Option extends Administrator_Controller
{
    ...
    */
    public function create()
    {
        ...
        $this->data['form_field']['general'] = [
            'type' => [
                ...
                'placeholder' => translate('form_label_type'),
                'options' => $options,
        ...
        $this->data['option_value'] = $this->input->post('option_value');
        ...
        //$this->form_validation->set_rules('option_value', translate('form_label_option_values'), 'required');
        ...
        if ($this->input->method() == 'post') {
            if($this->form_validation->run() == true) {
                ...
                if($id) {
                    ...
                    if($this->input->post('option_value')) {
                        foreach($this->input->post('option_value') as $option_value) {
    
                            $general = [
                                'option_id' => $id,
                                'image'     => $option_value['image'],
                                'value'     => $option_value['value'],
                                'sort'      => $option_value['sort_order']
                            ];
    
                            $option_value_id = $this->{$this->model}->insert_option_value($general);
                            if($option_value_id) {
                                if($option_value['option_value_description']) {
                                    foreach($option_value['option_value_description'] as $lang_id => $option_description) {
                                        ...
                                        $this->{$this->model}->insert_option_value_translation($option_value_translation);
                                    }
                                }
                            }
                        }
            ...
            } elseif(!$this->input->post('option_value')) {
                $this->data['message'] = translate('error_message_option_value_required');
            } else {
    ...
    public function edit($id)
    {
        ...
        $option = $this->{$this->model}->filter(['id' => $id])->one();
        ...
        if($option) {
            ...
            $this->data['form_field']['general'] = [
                'type' => [
                    ...
                    'placeholder' => translate('form_label_type'),
                    'options' => $options,
                    'selected' => (set_value('type')) ? set_value('type') : $option->type,
                    'validation' => ['rules' => 'required']
                ],
                'sort' => [
                    'property'  => 'number',
                    ...
                    'name'      => 'sort',
                    'class'     => 'form-control',
                    'value'     => (set_value('sort')) ? set_value('sort') : $option->sort,
                    'label'     => translate('form_label_sort'),
                    'placeholder' => translate('form_label_sort'),
                    'validation' => []
                ],
                'status' => [
                    ...
                    'label' => translate('form_label_status'),
                    'options' => [translate('disable', true), translate('enable', true)],
                    'selected' => (set_value('status')) ? set_value('status') : $option->status,
                    'validation' => ['rules' => 'required']
                ]
            ];
            ...
            // Translation Form Fields
            foreach ($this->data['languages'] as $language) { 
                $row_translation = $this->{$this->model}->filter(['option_id' => $id])->with_translation($language['id'])->one();
            ...
            $option_value = [];
            $rows = $this->{$this->model}->get_option_value($id);
            
            if($rows) {
                foreach ($rows as $row) {
                    $option_value_translations = $this->{$this->model}->get_option_value_translation($row->id);
                    if($option_value_translations) {
                        $option_value_description = [];
                        foreach($option_value_translations as $option_value_translation) {
                            $option_value_description[$option_value_translation->language_id] = ['name'=>$option_value_translation->name];
                        }
                    }
            ...
            $this->data['option_value'] = ($this->input->post('option_value')) ? $this->input->post('option_value') : $option_value;
            ...
            if ($this->input->method() == 'post') {
                if($this->form_validation->run() == true) {
                    ...
                    // Delete option value
                    $this->{$this->model}->delete_option_value($id);
                    $this->{$this->model}->delete_option_value_translation($id);
                    // Insert option value
                    if($this->input->post('option_value')) {
                        foreach($this->input->post('option_value') as $option_value) {
